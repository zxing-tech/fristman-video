...
- `lib/data/{services,industries,our-work}.ts` — typed arrays. `services` is read by `app/sitemap.ts` **and** by the navbar Services dropdown; `industries` and `our-work` are read by `sitemap.ts` alone. The listing pages hardcode their own copy of the cards in the page body, so adding or renaming a route means editing *both* the page and the data file, or the sitemap silently drifts from the site.
